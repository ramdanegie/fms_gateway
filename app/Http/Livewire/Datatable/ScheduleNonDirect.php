...
class ScheduleNonDirect extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if($this->filter_status == "EX") {
            $source = \App\Models\ExportTransit::whereRaw("1=1");
        ...
        if($this->filter_status == "IM") {
            $source = \App\Models\ImportTransit::whereRaw("1=1");
            if(!empty($this->filter_dest)) {
                $source->where("destination", "like", "%{$this->filter_dest}%");
            }
        ...
        return view('livewire.datatable.schedule-non-direct', [
            "items"     => $items,
            "data_from" => $data_from,
            "data_to"   => $data_to,
            "data_all"  => $data_all,
